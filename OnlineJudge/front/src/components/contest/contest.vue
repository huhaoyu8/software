...
<template>
  ...
  <el-space style="float: right">
    ...
    <el-switch
        ...
        active-text="显示标签"
        inactive-text="隐藏标签"/>
    <el-input placeholder="请输入……" style="width: 200px;height: 32px;margin-right: 50px">
      <template #append>
  ...
  <el-table :data="contestList" style="width: 100%;margin-top: 50px">
    <el-table-column label="比赛编号" prop="id" width="100px"/>
    <el-table-column label="比赛名称" prop="title">
      <template #default="item">
        <el-button @click="this.$router.push('/contest/' + item.row.id)" style="font-size: 20px" type="primary" text link>{{item.row.title}}</el-button>
      </template>
    </el-table-column>
    <el-table-column label="开始日期" prop="begin" />
    <el-table-column label="结束日期" prop="end"/>
  </el-table>
